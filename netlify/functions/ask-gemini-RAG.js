...
exports.handler = async function (event) {
  ...
  try {
    ...
    const queryResponse = await pineconeIndex.query({
      ...
      topK: 10,
      includeMetadata: true,  // <== Important for metadata to come through
    });

    // Log all matches safely and print preview
    ...
    queryResponse.matches.forEach((match, i) => {
      ...
      const previewText =
        match.metadata && typeof match.metadata.text === "string" && match.metadata.text.length > 0
          ? match.metadata.text.slice(0, 300)
          : "[Missing or empty text in metadata]";
      console.log(`Preview: ${previewText}`);
    });

    // Filter relevant matches safely (score + metadata.text check)
    ...
    if (relevantMatches.length === 0) {
      return {
        ...
        body: JSON.stringify({
          answer:
            "I couldn't find a rule in the documents that was a close enough match to answer that question. Please try rephrasing it.",
        }),
      };
    }

    // Build context from safe metadata.text fields only
